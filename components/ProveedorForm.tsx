...
import { Proveedor, ProveedorFormData } from '../types.js';

interface ProveedorFormProps {
  onSubmit: (data: ProveedorFormData | Proveedor) => void;
  onClose: () => void;
  initialData?: Proveedor;
}

const ProveedorForm: React.FC<ProveedorFormProps> = ({ onSubmit, onClose, initialData }) => {
  const [formData, setFormData] = useState<ProveedorFormData | Proveedor>(
    initialData || {
      nombre_proveedor: '',
    }
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <label htmlFor="nombre_proveedor" className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
        ...
        <input
          ...
          autoFocus
        />
        <p className="mt-1 text-xs text-gray-500 dark:text-slate-400">Ej: Mercado Los Chinos, Carretillero Juan</p>
      </div>
